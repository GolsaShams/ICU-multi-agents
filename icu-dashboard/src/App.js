...
function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError('Cannot reach backend at http://127.0.0.1:5000/view. Make sure Flask is running.');
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-700">
        <div className="text-center">
          <p className="text-2xl font-bold mb-2">Initializing System...</p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-700">
        <div className="text-center max-w-md">
          <p className="text-2xl font-bold mb-2">Dashboard Offline</p>
          <p className="text-sm text-red-500 mb-3">{error}</p>
          <p className="text-sm text-slate-500">
            In a terminal, run:
            <br />
            <code>py icu-dashboard/app.py</code>
          </p>
        </div>
      </div>
    );
            ...
            {data.patients.map((p) => (
              <div
                ...
                className={`bg-white p-6 rounded-2xl shadow-lg border-t-4 ${
                  p.status === 'Critical' ? 'border-red-500' : 'border-indigo-500'
                }`}
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 rounded-xl">
                    ...
                    <p
                      className={`text-3xl font-bold ${
                        p.hr > 100 ? 'text-red-500' : 'text-slate-800'
                      }`}
                    >
                      {p.hr}
                    </p>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl">
                    ...
                    <p
                      className={`text-3xl font-bold ${
                        p.temp > 38.0 ? 'text-red-500' : 'text-slate-800'
                      }`}
                    >
                      {p.temp}
                    </p>
          ...
          <div className="bg-slate-900 rounded-2xl p-4 h-[500px] overflow-y-auto shadow-2xl">
            {data.alerts.length === 0 ? (
              ...
              data.alerts.map((alert) => (
                <div
                  key={alert.id}
                  className="mb-4 p-4 border-l-2 border-red-500 bg-slate-800 rounded-r-lg"
                >
